  ...
  if (isWindows() || isWSL()) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        // biome-ignore lint/style/noNonNullAssertion: candidate is validated by exec check
        return candidate!
      } catch {}
    }
  }
...
export async function httpGet(
  ...
  try {
    ...
    clearTimeout(timer)
    if (!resp.ok) return { ok: false, status: resp.status, json: async () => null }
...
export function killComet(): void {
  try {
    if (isWindows()) execSync('taskkill /F /IM comet.exe', { stdio: 'ignore' })
    else execSync("pkill -f 'Comet.app'", { stdio: 'ignore', shell: '/bin/bash' })
  } catch {}
}
